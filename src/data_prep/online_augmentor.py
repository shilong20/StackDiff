# -*- coding: utf-8 -*-
"""
在线数据增强管线（从 notebooks/augment_pipeline.ipynb 提炼的纯 Python 版本）。

功能要点：
- 读入灰度原图（0-255/0-1 均可）与 mask（0 表示空白）。
- 处理顺序：弹性/透视 → 碳堆积(概率) → 旋转(同步旋转距离图) → 安全裁剪并缩放至128 → 翻转 → 扫描/泊松噪声 → 轻量显示风格变换。
- 最终输出 128×128，float32，[-1, 1]，单通道（C=1）。

超参数采样规则（与需求对齐）：
- 标注“建议范围”的参数：在范围内均匀采样。
- 标注“默认值”的参数：固定该值；若同时存在建议范围，则按建议范围采样。
- 碳堆积启用概率固定为 0.01（可由 cfg.carbon.prob 控制）。

注意：
- 裁剪安全性由 mask 的距离变换保障：先对 mask 做距离变换，再与图片一同旋转，选择距离>safe_radius 的裁剪中心。
- 图像边界操作使用反射填充，避免“黑边”。
"""
...
_DEF_E = 1.602176634e-19  # 电子电荷（库仑）
_SUPPORTED_DISABLES = {
    "elastic",
    "perspective",
    "carbon",
...
def choose_crop(dist_map: np.ndarray, side: int, safe_radius: int) -> Tuple[int, int]:
    ...
    if H <= 2 * half or W <= 2 * half:
        raise RuntimeError(f"图像尺寸过小，无法裁剪 side={side}，尺寸={(H, W)}")
    ...
    if cand.size == 0:
        # 放宽：允许以 side/2 作为阈值再次尝试
        allowed = dist_map > float(side / 2.0)
        cand = np.argwhere(allowed & border)
        if cand.size == 0:
            raise RuntimeError("未找到合法裁剪中心，请检查 mask 或参数。")
...
def apply_edge_mask(
    ...
    - bg_min: 遮罩区域背景填充最小值（相对于0-1范围），默认0.0。
    - bg_max: 遮罩区域背景填充最大值（相对于0-1范围），默认0.05。
    ...
    - 总体概率约 1-(1-prob)^4 ≈ 0.002（平均每500张约1张触发）
    - 遮罩区域每个像素独立填充 uniform(bg_min, bg_max) 的随机值，模拟无原子但有背景信号的真实情况
    - 后续噪声（泊松/高斯）会叠加到这些背景值上，确保物理真实性
    """
    img = np.asarray(image, dtype=np.float32)
    h, w = img.shape[:2]
...
def _maybe_sample(value: Any) -> Any:
    """
    采样规则：
    - 若为标量（int/float/bool/None/str），直接返回。
    - 若为形如 [lo, hi] 的列表/元组（长度2，均为数值），则一律视为“建议范围”，做均匀采样（float）。
    - 若为其他容器，原样返回。
    """
...
class AugmentConfig:
    ...
    def sample_once(self) -> Dict[str, Any]:
        ...
        if elastic_enabled:
            ...
            out["elastic.sigma_ratio"] = el.get("sigma_ratio", 0.05)  # 默认固定
        else:
        ...
        # focus_center 可给定 [lo,hi]，对 x/y 各采样一次
...
class OnlineAugmentor:
    ...
    def __call__(self, img: Image.Image | np.ndarray) -> np.ndarray:
        """
        对单张原始灰度图执行一次随机增强，返回 1×H×W 的 float32 [-1,1] 张量（numpy 数组）。
        """
        ...
        # 6) 边缘遮罩（在噪声前施加，使遮罩区域也受噪声和显示调整影响）
