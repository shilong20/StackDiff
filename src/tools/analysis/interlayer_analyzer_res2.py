...
"""
【作用概述】
对一个“包含多个双层分解子文件夹”的 root 目录做层间关系解析：
1) 先调用 `tools/pred_dadb/pipeline_bilayer_root.py`（或复用已有输出）得到每个样本的：
   - 两层的原点 origin（用于 flip 轴；512 坐标系）
   - 两层的 (da,db)（512 坐标系）
   - 两层的原子点云（从 pipeline 落盘的 atoms.json 读取；坐标系与 dadb 一致，512）
   - 粗分类 class：slip/twist/flip_slip/flip_twist/unknown
2) 再基于这些结果计算下游量：
   - slip：粗到细的平移搜索得到 shift_px（512 像素坐标系输出），并投影到两层的 (da,db) 坐标求 (a,b) 系数与规约；
   - twist：用“位移向量集合匹配”的旋转搜索输出精确扭角 twist_fine_deg（范围 [0,180)）；
   - flip_twist：不做下游量计算；
   - flip_slip：先把 layer0 的原子点云沿对称轴反射（对称轴过 pipeline 输出的 origin0，且垂直于 db_mean），
               将其转为等价的 non-flip slip 后按 slip 流程计算 shift 与 (a,b)。

核心输入/输出：
- 输入：`--root <dir>`，其下每个一级子目录视为一个双层样本文件夹（里面一般有 4 张图，*_0.png/*_1.png 为单层图）。
- 输出：`--out_csv <path>`，每个双层样本一行：包含类别、粗/精 twist、slip/flip_slip 的 shift 与 (a,b) 投影结果等。
  说明：slip 的 shift 约定与旧实现一致：搜索得到的是“需要施加到 layer1 点云上使其对齐 layer0”的 t_align，
  最终输出的 shift_px 定义为 `shift_px = -t_align`（即 layer0->layer1 的位移）。

【关联说明】文件/模块：
- tools/pred_dadb/pipeline_bilayer_root.py（批量双层 dadb/原点/原子点云输出：本脚本会优先复用其输出）
- tools/pred_dadb/pipeline_single.py（单张图 dadb/原点提取：由 pipeline_bilayer_root 间接调用）

【命令行用法】
python src/tools/analysis/interlayer_analyzer_res2.py --root data/HighT1 --out_csv tools/pred_dadb/highT1_interlayer_from_pred.csv
python src/tools/analysis/interlayer_analyzer_res2.py --root data/HighT1 --out_csv tools/pred_dadb/highT1_interlayer_from_pred.csv --force_pred
（参数：默认会在发现 pred_dadb 输出已存在时跳过 pipeline；--force_pred=强制重跑 pred_dadb pipeline；--pred_csv/--pred_atoms_dir=显式指定 pred_dadb 输出；--slip_max_shift_px=slip 平移搜索最大范围（512 坐标系））
"""
...
_PRED_PIPELINE = _REPO_ROOT / "tools" / "pred_dadb" / "pipeline_bilayer_root.py"


def _parse_sideA_A_from_folder(name: str) -> Optional[float]:
    """从文件夹名末尾解析 `_{a}x{b}`（nm），返回平均边长（Å）。"""
...
def _angle_deg_directed_0_180(u: np.ndarray, v: np.ndarray) -> float:
    """有向向量夹角（不取 abs）：arccos(cos) -> [0,180]（度）。"""
    u = np.asarray(u, dtype=np.float64).reshape(2)
    v = np.asarray(v, dtype=np.float64).reshape(2)
    nu = float(np.hypot(u[0], u[1]))
    nv = float(np.hypot(v[0], v[1]))
...
def estimate_translation_multiscale_robust(
    ...
    *,
    ...
    """
    粗到细的整像素平移搜索（鲁棒 trimmed-mean 最近邻评分版本）。
    返回：(t_align_xy, best_score)，其中 t_align 是“施加到 pts_mov 上使其对齐 ref”的平移。
    """
...
def _align_basis_sign_by_db(da: np.ndarray, db: np.ndarray, *, db_ref: np.ndarray) -> Tuple[np.ndarray, np.ndarray, int]:
    """
    若 db 与 db_ref 反向，则整体翻转 (da,db)->(-da,-db) 以消除 180° 符号歧义。
    返回：(da2, db2, s)，其中 s=+1 表示未翻转，-1 表示整体取反。
    """
    da = np.asarray(da, dtype=np.float64).reshape(2)
    db = np.asarray(db, dtype=np.float64).reshape(2)
...
def _canonicalize_basis_by_db_halfplane(da: np.ndarray, db: np.ndarray) -> Tuple[np.ndarray, np.ndarray, int]:
    """
    用 db 的半平面约定消除整体 180° 符号歧义：
    - 若 db_x < 0 或 (db_x==0 且 db_y < 0)，则整体翻转 (da,db)->(-da,-db)
    返回：(da2, db2, s)；s=+1 表示未翻转，-1 表示整体取反。
    """
    da = np.asarray(da, dtype=np.float64).reshape(2)
    db = np.asarray(db, dtype=np.float64).reshape(2)
...
def _align_coeffs_mod1(c_ref: np.ndarray, c_other: np.ndarray) -> np.ndarray:
    """
    在 Z^2 平移等价下对齐：返回 c_other_adj = c_other - round(c_other-c_ref)，
    使每个分量与 c_ref 的差尽量落在 [-0.5,0.5] 附近，便于后续平均。
    """
...
def _reduce_coeffs_res2(c: np.ndarray) -> np.ndarray:
    ...
    ReS2 规约：
    - 先把每个分量折叠到 [0,1)
    - 再强制 c_a + c_b <= 1：若超出则映射到 (1-c_a,1-c_b)
    """
...
def estimate_twist_deg_from_vecs_search(
    ...
    *,
    ...
    """
    用“位移向量集合”的最近邻距离估计相对旋转（不依赖 PBC，也不输出平移）。
...
def _reflect_vec_about_axis(v: np.ndarray, axis_dir: np.ndarray) -> np.ndarray:
    """反射向量 v 关于“过原点、方向为 axis_dir 的直线”（只作用于向量，不含平移）。"""
...
def _reflect_points_about_axis(points: np.ndarray, *, origin: np.ndarray, axis_dir: np.ndarray) -> np.ndarray:
    """反射点集关于“过 origin、方向为 axis_dir 的直线”。"""
...
def _default_pred_paths(out_csv: Path) -> Tuple[Path, Path]:
    pred_csv = out_csv.with_name(f"{out_csv.stem}__pred_dadb.csv")
    pred_atoms_dir = out_csv.with_name(f"{out_csv.stem}__pred_atoms")
    return pred_csv, pred_atoms_dir


def _run_pred_pipeline_if_needed(
    *,
    root: Path,
    pred_csv: Path,
    pred_atoms_dir: Path,
    ...
    pred_atoms_dir = Path(pred_atoms_dir)
    idx = pred_atoms_dir / "atoms_index.json"
    if (not bool(force_pred)) and pred_csv.exists() and idx.exists():
        return
...
def _load_pred_outputs(pred_csv: Path, pred_atoms_dir: Path) -> Tuple[Dict[str, Dict[str, str]], Dict[str, Path], float]:
    pred_rows: Dict[str, Dict[str, str]] = {}
    with pred_csv.open(encoding="utf-8") as f:
        for r in csv.DictReader(f):
            pred_rows[str(r.get("folder", ""))] = dict(r)

    atoms_index_path = pred_atoms_dir / "atoms_index.json"
    ...
    for it in atoms_index:
        ...
        if img and fn:
            img2json[img] = pred_atoms_dir / fn
    ...
    if out_scale is None:
        out_scale = 4.0
    return pred_rows, img2json, float(out_scale)
...
def main() -> int:
    ...
    ap.add_argument("--root", required=True, help="包含多个双层样本子文件夹的根目录")
    ap.add_argument("--out_csv", required=True, help="下游分析 CSV 输出路径")
    ap.add_argument("--limit", type=int, default=0)

    # pred_dadb reuse / cache
    ap.add_argument("--force_pred", action="store_true", help="强制重跑 pred_dadb pipeline（默认若输出已存在则跳过）")
    ap.add_argument("--pred_csv", default="", help="pred_dadb 的输出 CSV（默认根据 out_csv 自动推断）")
    ap.add_argument("--pred_atoms_dir", default="", help="pred_dadb 的 atoms 目录（默认根据 out_csv 自动推断）")
    ap.add_argument("--pred_out_scale", type=float, default=4.0, help="pred_dadb 输出坐标缩放倍数（默认 4：128->512）")

    # atom detect params (passed through to pred pipeline)
    ...
    ap.add_argument("--sideA_A", type=float, default=27.9, help="若无法从文件夹名解析视野尺寸，则使用该默认值（Å）")

    # slip translation search (512 coord)
    ap.add_argument("--slip_max_shift_px", type=int, default=48, help="最大平移搜索范围（512 坐标系像素）")
    ...
    ap.add_argument("--twist_r_min_px", type=float, default=8.0, help="位移向量最小半径（512 坐标系像素）")
    ap.add_argument("--twist_r_max_px", type=float, default=160.0, help="位移向量最大半径（512 坐标系像素）")
    ...
    ap.add_argument(
        ...
        type=float,
        default=30.0,
        help="twist 精细搜索时围绕 db 有向夹角的局部搜索半径（<=0 表示全局 0~180 搜索）",
    )
    ap.add_argument(
        "--twist_augment_sign",
        action="store_true",
        default=False,
        help="是否对位移向量集合做 ±v 增广（可能引入额外对称性；默认关闭）",
    )
    args = ap.parse_args()
    ...
    pred_atoms_dir_arg = str(args.pred_atoms_dir).strip()
    pred_csv, pred_atoms_dir = _default_pred_paths(out_csv)
    if pred_csv_arg:
        pred_csv = Path(pred_csv_arg)
    if pred_atoms_dir_arg:
        pred_atoms_dir = Path(pred_atoms_dir_arg)
    ...
    _run_pred_pipeline_if_needed(
        root=root,
        pred_csv=pred_csv,
        pred_atoms_dir=pred_atoms_dir,
        limit=int(args.limit),
        slip_thresh_deg=float(args.slip_thresh_deg),
        out_scale=float(args.pred_out_scale),
        force_pred=bool(args.force_pred),
        use_highpass=use_hp,
    ...
    pred_rows, img2json, pred_out_scale = _load_pred_outputs(pred_csv, pred_atoms_dir)
    ...
    for d in dirs:
        ...
        def _maybe_scale_pts(pts: Optional[np.ndarray]) -> Optional[np.ndarray]:
            if pts is None or pts.size == 0:
                return pts
            m = float(np.max(pts))
            if m <= 140.0 and float(pred_out_scale) > 1.0:
                return (pts * float(pred_out_scale)).astype(np.float64)
        ...
        if cls == "unknown" or (not ok_for_downstream):
            ...
            if cls == "flip_slip":
                n_flip_slip += 1
                # 以 db_mean 为基准确定翻转轴（轴过 origin0，且 ⟂ db_mean）。
                # 若缺失 db_mean，则退化回 db0。
        ...
        row = {
            ...
            "pred_theta_deg": coarse_twist,
            "pred_theta_method": theta_method,
            "class": cls,
            "reason": reason,
            "is_flip": is_flip,
