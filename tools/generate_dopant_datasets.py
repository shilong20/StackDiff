...
def main() -> int:
    ...
    ap.add_argument(
        "--output-prefix",
        default="ReS2_dopant",
        help="输出目录名前缀（默认 ReS2_dopant，将生成 ReS2_dopant0.05 等）",
    )
    ...
    for r in rates:
        ...
        run_data["dopant_rates_re"] = [float(r)]
        run_data["defect_rates_re"] = [0.0]
        run_data.pop("rotation_range", None)
        if args.num is not None:
            run_data["num"] = int(args.num)
        out_dir = output_parent / f"{args.output_prefix}{_fmt_rate(r)}"
        ...
        try:
            ...
            print(f"\n[RUN] dopant_rates_re={[float(r)]} output_dir={out_dir}  [{noise_info}]")
