#!/usr/bin/env python3
"""
Purpose: Analyze interlayer geometry for folders of separated bilayer ReS2 outputs. It reuses or runs the da/db pipeline, then reports slip shifts, da/db coordinate projections, and fine twist angles in a CSV file.
Related files: tools/pred_dadb/pipeline_bilayer_root.py, tools/pred_dadb/pipeline_single.py, and README.md.
CLI usage: python tools/analyze_interlayer_res2.py --root outputs/ReS2 --out_csv outputs/res2_interlayer.csv (arguments: --root=input folders; --out_csv=analysis CSV; --force_pred reruns da/db prediction).
"""
...
def main() -> int:
    ...
    ap.add_argument("--force_pred", action="store_true", help="Force rerunning the da/db pipeline instead of reusing existing outputs.")
    ap.add_argument("--pred_csv", default="", help="Existing or target pred_dadb CSV path. Inferred from out_csv by default.")
    ap.add_argument("--pred_atoms_dir", default="", help="pred_dadb atoms directory; inferred from out_csv by default")
    ap.add_argument("--pred_out_scale", type=float, default=4.0, help="Coordinate scale for pred_dadb outputs. The default maps 128 px to 512 px.")
